...
my.plot <- reactive ({
  ...
  my.plot <-  ggplot(
    ...
    ggtitle(paste("Locations given in NZTM based on a ", 
...
* **Aggregate points:** This widget allows the user to aggregate locations within a certain distance of each other. Locations are changed to the NZ Transverse Mercator grid (NZTM) to facilitate better distance measurement. The new NZTM location which labels any given location is the centroid of the points within the aggregation distance.   As the NZTM locations are esoteric any given positions can be manually entered into the **Flag Position** input boxes, the point will be then be revealed as a blue marker on the *Select area* tab.
...
* **Data extract tab:** a simplified version of the data within the selected area is shown on screen for quick exploratory work.
   ...
   * The data from the selected area can be downloaded as a more comprehensive csv file by clicking on the **Download as csv file** button.
...
The dashboard was constructed under R version 4.2.0. The final project will be available from github and be licensed under a permissive MIT license. Note the base maps used in the dashboard are controlled by a 3rd party and can be discontinued by them without notice. The availability of different maps can be checked [here](https://leaflet-extras.github.io/leaflet-providers/preview/).
...
renderUI({
  ...
  check2 <- paste("Names of organisations contributing: ",
    paste(unique(APIs$`Shared APIs`), collapse = ", "))
  
  check3 <- paste("Number of APIs: ", nrow(APIs))
    
  HTML(paste(check1, check2, check3, sep="<br/>"))
